...
app.post('/api/search', async (req, res) => {
  try {
    ...
    // Prefix-based fetch to handle truncated POs in SI (first 7 chars)
    let invoices = [];
    try {
      const norm = String(query || '').toUpperCase().replace(/[\s-]+/g, '');
      const prefix = norm.slice(0, 7);
      invoices = await sportsInc.getInvoicesByPrefix(prefix, 7);
      if ((!invoices || invoices.length === 0) && norm.length >= 6) {
        // Fallback to 6-char prefix if 7 yields nothing
        invoices = await sportsInc.getInvoicesByPrefix(norm.slice(0, 6), 6);
      }
    } catch (err) {
      console.error('Sports Inc fetch error (prefix):', err.message);
    }
